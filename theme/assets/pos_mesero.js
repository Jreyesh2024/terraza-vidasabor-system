  (function () {
    ...
    function getBebidaSubcategoria(prod) {
      ...
      var sku = (prod.codigo_sku || '').toLowerCase();

      // 1. Tés e infusiones prioritarios por nombre (Té Matcha, Té Verde, Tisana, Infusión, etc.)
      if (/\b(té|tés|matcha|tisana|tisanas|infusión|infusion|infusiones|manzanilla|chai|herbal)\b/i.test(nom) || /^té\b/i.test(nom)) {
        return 'tes';
      }

      // 2. Cafés prioritarios por nombre (Café, Espresso, Latte, Capuchino, Americano, etc.)
      if (/\b(café|cafe|espresso|espressos|latte|lattes|capuchino|capuchinos|cappuccino|cappuccinos|americano|americanos|macchiato|moka|mocha)\b/i.test(nom)) {
        return 'cafes';
      }

      // 3. Por código SKU
      if (sku.includes('caf')) return 'cafes';
      if (sku.includes('tea') || sku.includes('tis') || sku.includes('inf') || sku.includes('tev')) return 'tes';

      // 4. Por descripción
      if (/\b(matcha|tisana|tisanas|infusión|infusion|manzanilla|chai)\b/i.test(desc)) {
        return 'tes';
      }
      if (/\b(café|cafe|espresso|latte|capuchino|cappuccino|americano)\b/i.test(desc)) {
        return 'cafes';
      }

      // 5. Todo lo demás -> Bebidas & Refrescos
      return 'bebidas';
    }
    ...
    function _vsBadgeItem(item) {
      ...
      var colores = {
        'borrador':       ['linear-gradient(135deg,#475569,#334155)', '#cbd5e1', '📝 En Borrador'],
        'buffer':         ['linear-gradient(135deg,#b45309,#78350f)', '#fef3c7', '⏳ En Buffer (5 min)'],
      ...
      return '<span style="display:inline-block;padding:2px 8px;border-radius:999px;'
             + 'background:' + c[0] + ';color:' + c[1] + ';font-size:9.5px;'
             + 'font-weight:800;letter-spacing:0.02em;'
             + 'box-shadow:0 2px 5px rgba(0,0,0,0.35);">' + c[2] + '</span>';
    }
    ...
    function _vsTooltipHTMLSilla(mesa, silla) {
      ...
      if (estado !== 'ocupada' && items.length === 0) {
        return _vsTarjetaBase(
          ...
          + '  <div style="color:#e5e7eb;font-size:12px;font-weight:700;">Disponible para nuevo cliente</div>'
          + '  <div style="color:#94a3b8;font-size:10px;margin-top:4px;">'
          + '    Escanea el QR o clic para asignar</div>'
          + '</div>'
        );
      }
      ...
      if (items.length === 0) {
        lineas = '<div style="padding:16px;text-align:center;">'
               ...
               + '    Ocupada SIN comanda</div>'
               + '  <div style="color:#94a3b8;font-size:10px;margin-top:4px;">'
               + '    Cliente aún no solicita alimentos</div>'
               + '</div>';
      } else {
        var lineasArr = [];
        ...
        lineas = '<div style="max-height:220px;overflow-y:auto;">' + lineasArr.join('') + '</div>';
        if (items.length > 8) {
          lineas += '<div style="text-align:center;color:#94a3b8;font-size:10px;padding:6px;">'
                  + '… y ' + (items.length - 8) + ' platillos más</div>';
        }
      }
      ...
      var body = ''
        ...
        + '  <div style="display:flex;justify-content:space-between;align-items:center;">'
        + '    <div style="color:#f8fafc;font-size:11.5px;font-weight:700;">👤 ' + comensal + '</div>'
        ...
        + (items.length > 0
          ? '<div style="display:flex;justify-content:space-between;align-items:center;'
          +   'padding:10px 12px;background:linear-gradient(180deg,#020617,#0f172a);'
          +   'border-top:2px solid #10b981;">'
          + '  <span style="color:#94a3b8;font-size:11px;font-weight:700;">Total silla</span>'
          + '  <span style="color:#10b981;font-weight:900;font-size:15px;">$' + total.toFixed(2) + '</span>'
          + '</div>' : '');
    ...
    function _vsTarjetaBase(gradienteHeader, colorAccento, titulo, subtitulo, bodyHTML) {
      return ''
        + '<div style="background:#0f172a;border-radius:18px;overflow:hidden;'
        +   'border:2px solid ' + colorAccento + ';box-shadow:0 20px 50px rgba(0,0,0,0.85);">'
        + '  <div style="background:' + gradienteHeader + ';padding:10px 14px;">'
        + '    <div style="font-weight:900;font-size:13.5px;color:#fff;'
        +       'letter-spacing:0.02em;">' + titulo + '</div>'
        + '    <div style="font-size:10px;font-weight:900;color:' + colorAccento + ';'
        +       'margin-top:1px;letter-spacing:0.06em;">' + subtitulo + '</div>'
        + '  </div>'
        + bodyHTML
        + '</div>';
    }
    ...
    function _vsMiniStat(n, color, label) {
      return '<div style="text-align:center;padding:5px 2px;background:#0f172a;'
           +   'border-radius:6px;border:1px solid #1e293b;">'
           +   '<div style="font-size:14px;font-weight:900;color:' + color + ';">' + n + '</div>'
           +   '<div style="font-size:7.5px;color:#94a3b8;font-weight:800;'
    ...
    function _vsTooltipHTMLMesa(mesa) {
      ...
      Object.keys(cuentas).forEach(function (k) {
        ...
        if (!c) return;
        var parts = k.split('-');
        var mId = parseInt(c.mesaId) || parseInt(parts[0]);
        var sId = (c.sillaId !== undefined) ? parseInt(c.sillaId) : (parts.length > 1 ? parseInt(parts[1]) : 0);
        if (mId !== parseInt(mesa)) return;

        var isCentro = (sId === 0 || c.esAlCentro === true || c.es_cuenta_mesa === true || k.endsWith('-0'));
        if (isCentro) {
          var itemsC = c.items || [];
          itemsC.forEach(function (i) {
            var sub = (parseFloat(i.precio) || 0) * (parseInt(i.cantidad) || 1);
            totalAlCentro += sub;
            itemsAlCentro.push(i);
          });
        }
      });
      ...
      // SI HAY PEDIDOS AL CENTRO DE LA MESA
      if (itemsAlCentro.length > 0) {
        var gradiente = 'linear-gradient(135deg,#d97706,#b45309)';
        var acento = '#fde68a';
        var subtitulo = '🍲 ' + itemsAlCentro.length + ' PLATILLO(S) AL CENTRO';

        var totalBanner = '<div style="display:flex;justify-content:space-between;align-items:center;background:#020617;padding:8px 12px;border-bottom:1px solid #1e293b;">'
                        + '  <span style="color:#94a3b8;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:0.04em;">Total al Centro:</span>'
                        + '  <span style="color:#10b981;font-size:14px;font-weight:900;">$' + totalAlCentro.toFixed(2) + '</span>'
                        + '</div>';

        var centroRows = '';
        itemsAlCentro.forEach(function (it) {
          var sub = (parseFloat(it.precio) || 0) * (parseInt(it.cantidad) || 1);
          centroRows += '<div style="padding:8px 12px;border-bottom:1px solid #1e293b;display:flex;flex-direction:column;gap:4px;">'
                      + '  <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;">'
                      + '    <div style="flex:1;min-width:0;font-size:12px;color:#fef08a;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'
                      + '      <span style="color:#fbbf24;">' + (it.cantidad || 1) + '×</span> ' + (it.nombre || '?')
                      + '    </div>'
                      + '    <div style="font-size:12px;font-weight:900;color:#10b981;">$' + sub.toFixed(2) + '</div>'
                      + '  </div>'
                      + (it.notas ? '<div style="font-size:10px;color:#fde047;font-style:italic;">Nota: ' + it.notas + '</div>' : '')
                      + '  <div>' + _vsBadgeItem(it) + '</div>'
                      + '</div>';
        });

        var lista = '<div style="max-height:180px;overflow-y:auto;">' + centroRows + '</div>';
        var pie = '<div style="padding:8px;text-align:center;color:#94a3b8;font-size:10px;font-weight:700;background:#020617;border-top:1px solid #1e293b;">'
                + 'Clic en la mesa para gestionar pedido al centro</div>';

        return _vsTarjetaBase(gradiente, acento, titulo, subtitulo, totalBanner + lista + pie);
      }

      // SI NO HAY PEDIDO AL CENTRO DE LA MESA
      var gradienteVacio = 'linear-gradient(135deg,#334155,#1e293b)';
      var acentoVacio = '#94a3b8';
      var subtituloVacio = '○ SIN PEDIDO AL CENTRO';
      var cuerpoVacio = '<div style="padding:22px 16px;text-align:center;">'
                      + '  <div style="font-size:30px;margin-bottom:6px;">🍲</div>'
                      + '  <div style="color:#f1f5f9;font-size:13px;font-weight:800;">No hay pedido al centro de la mesa</div>'
                      + '  <div style="color:#94a3b8;font-size:10.5px;margin-top:4px;">Clic en la mesa para agregar platillos compartidos (Al Centro)</div>'
                      + '</div>';

      return _vsTarjetaBase(gradienteVacio, acentoVacio, titulo, subtituloVacio, cuerpoVacio);
    }
    ...
    window.installHoverTooltips = function () {
      ...
      root.addEventListener('mouseover', function (ev) {
        ...
        if (sillaEl) {
          ...
          if (!m && sillaEl.id) {
            var parts = sillaEl.id.split('-');
            if (sillaEl.id.indexOf('extra-chair-') === 0 && parts.length === 3) {
              m = '0';
              s = parts[2];
        ...
        if (mesaEl) {
          ...
          if (mesaEl.dataset.args) {
            mesaNum = mesaEl.dataset.args.trim();
          } else if (mesaEl.id) {
            var mparts = mesaEl.id.split('-');
